...
<template>
    <div class="w-full fixed bg-white top-0 left-0">
        <div class="flex-row justify-between flex flex-1 px-5 py-3 lg:max-w-5xl lg:mx-auto ">
            ...
            <div @click="searchNavClicked" class="transition flex justify-center items-center w-9/12 sm:w-6/12 md:w-7/12 h-10 rounded-lg hover:bg-slate-400 active:bg-slate-200 bg-slate-200 text-pretty ">
                <FontAwesomeIcon class="mx-3 w-3 h-3" :icon="['fas', 'magnifying-glass']" />
                <div class="text-sm ">{{ indexHeaderStore.stayInputHeaderValue }}</div>
            </div>
            <div class="h-full">
                <UiPrimaryButton title="Sign In" />
            </div>
        </div>
        <div @click="searchNavClicked"  class="fixed w-full h-full opacity-95 bg-slate-400 sm:px-5 rounded-md" :class="isModalShow ? '' : 'hidden'">
